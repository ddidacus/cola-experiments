...
def run(game, hyper_params, results_dir, output_dir):
    ...
    plt.savefig(f"{output_dir}/cola_gradient_field.png", bbox_inches='tight', dpi=300)
    plt.clf()

    ticks = np.linspace(-interval, interval, grain).round(2)

    plt.figure(figsize=(14, 12))
    ax = sns.heatmap(errors_cola, annot=True, fmt=".2f", annot_kws={"size": 6}, linewidth=0.5, xticklabels=ticks, yticklabels=ticks)
    ax.tick_params(labelsize=7)
    plt.title("Errors")
    plt.savefig(f"{output_dir}/cola_errors_heatmap.png", bbox_inches='tight', dpi=300)
    plt.clf()

    plt.figure(figsize=(14, 12))
    ax = sns.heatmap(x_comps_cola, annot=True, fmt=".2f", annot_kws={"size": 6}, linewidth=0.5, xticklabels=ticks, yticklabels=ticks)
    ax.tick_params(labelsize=7)
    plt.title("X Components")
    plt.savefig(f"{output_dir}/cola_x_components_heatmap.png", bbox_inches='tight', dpi=300)
    plt.clf()

    plt.figure(figsize=(14, 12))
    ax = sns.heatmap(y_comps_cola, annot=True, fmt=".2f", annot_kws={"size": 6}, linewidth=0.5, xticklabels=ticks, yticklabels=ticks)
    ax.tick_params(labelsize=7)
    plt.title("Y Components")
    plt.savefig(f"{output_dir}/cola_y_components_heatmap.png", bbox_inches='tight', dpi=300)
    plt.clf()
    ...
    plt.savefig(f"{output_dir}/lola_streamplot.png", bbox_inches='tight', dpi=300)
    plt.clf()
    plt.figure(figsize=(15, 8))
    plt.quiver(x, y, x_comps_lola, y_comps_lola)
    plt.title('LOLA')
    plt.savefig(f"{output_dir}/lola_gradient_field.png", bbox_inches='tight', dpi=300)
    plt.clf()
    ...
    plt.savefig(f"{output_dir}/hola4_gradient_field.png", bbox_inches='tight', dpi=300)
    plt.clf()
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Plot gradient fields for COLA, LOLA, and HOLA.')
